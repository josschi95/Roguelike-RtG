...
public class TESTSAVEJSON : MonoBehaviour
{
    ...
    public void TestSave()
    {
        ...
        {
            string parent = "";
            //if (archetypes[i] is CreatureSubType child) parent = child.parentArchetype.ArchetypeName;
            var type = new SpeciesArchetype(archetypes[i].TypeName, archetypes[i].NeedsAir, archetypes[i].NeedsFood, archetypes[i].NeedsSleep, parent);
        ...
        for (int i = 0; i < data.species.Length; i++)
        {
            ...
            race.archetype = races[i].Type.TypeName;
