...
public class Erosion : MonoBehaviour
{
    ...
    public void Erode(float[] map, int mapSize, int numIterations, int newSeed)
    {
        ...
        for (int iteration = 0; iteration < numIterations; iteration++)
        {
            ...
            for (int lifetime = 0; lifetime < maxDropletLifetime; lifetime++)
            {
                ...
                if (!WorldMap.instance.GetNode(nodeX, nodeY).isLand)
                {
                    break;
                }
    ...
    public float[,] Erode_2D(float[,] heightMap, int numIterations, int seed)
    {
        ...
        for (int iteration = 0; iteration < numIterations; iteration++)
        {
            ...
            for (int lifetime = 0; lifetime < maxDropletLifetime; lifetime++)
            {
                ...
                // Calculate droplet's height and direction of flow with bilinear interpolation of surrounding heights
                HeightAndGradient heightAndGradient = CalculateHeightAndGradient_2D(heightMap, posX, posY);
                ...
                // Find the droplet's new height and calculate the deltaHeight
                float newHeight = CalculateHeightAndGradient_2D(heightMap, posX, posY).height;
                float deltaHeight = newHeight - heightAndGradient.height;
    ...
    private HeightAndGradient CalculateHeightAndGradient_2D(float[,] heightMap, float posX, float posY)
    {
        int coordX = (int)posX;
        int coordY = (int)posY;
